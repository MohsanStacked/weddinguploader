...
const FileUploader = ({ coupleNames }: FileUploaderProps) => {
  ...
  useEffect(() => {
    const checkAndCreateBucket = async () => {
      ...
      try {
        console.log(
          "Checking Supabase connection with URL:",
          supabaseConfig.url
        );
        ...
        if (error) {
          ...
          if (error.message.includes("not found")) {
            ...
            if (createError) {
              ...
              setUploadError(
                "Failed to initialize storage. Please try again later."
              );
            ...
            setUploadError(
              "Failed to connect to storage. Please try again later."
            );
            setCheckingStorage(false);
            return;
          }
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (failedUploads > 0 && completedFiles === 0) {
        ...
        throw new Error(
          `Failed to upload any files: ${errorSummary}${additionalErrors}`
        );
        ...
        const additionalErrors =
          failedUploads > 3 ? `\n...and ${failedUploads - 3} more errors` : "";
        setUploadError(
          `${completedFiles} files uploaded successfully, but ${failedUploads} files failed:\n${errorSummary}${additionalErrors}`
        );
      } else {
        setUploadSuccess(true);
      }
    ...
    <div className="w-full max-w-md mx-auto bg-white/90 backdrop-blur-sm rounded-3xl shadow-lg p-8 my-8 border-2 border-[#e6d7bf] cute-shadow relative overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-16 h-16 opacity-40">
        <svg viewBox="0 0 100 100" className="w-full h-full">
          ...
          />
          <path
            d="M20,20 A10,10 0 0,1 30,10 A10,10 0 0,1 40,20 A10,10 0 0,1 30,40 A10,10 0 0,1 20,20"
            fill="#d2b48c"
          />
        </svg>
      </div>
      ...
      <div className="absolute top-0 right-0 w-16 h-16 opacity-40">
        <svg viewBox="0 0 100 100" className="w-full h-full">
          ...
          />
          <path
            d="M70,30 Q80,20 90,25 Q100,20 100,30 Q110,40 100,45 Q105,55 90,55 Q80,65 70,55 Q60,65 60,45 Q50,40 70,30"
            fill="#a98467"
          />
        </svg>
      </div>
      ...
      <div className="absolute bottom-0 right-0 w-16 h-16 opacity-40">
        <svg viewBox="0 0 100 100" className="w-full h-full">
          ...
          />
          <path
            d="M80,80 L85,65 L95,60 L85,55 L80,40 L75,55 L65,60 L75,65 Z"
            fill="#d4ac6e"
          />
        </svg>
      </div>
      ...
      <h3 className="text-center text-[#d2b48c] wedding-script text-2xl mb-4">
        Share Your Memories
      </h3>
      ...
      <div className="flex items-center justify-center mb-4">
        <div
          className={`w-3 h-3 rounded-full mr-2 ${
            bucketReady ? "bg-green-500" : "bg-red-500"
          }`}
        ></div>
          ...
          <div className="flex flex-col items-center justify-center space-y-3">
            ...
            <div className="w-12 h-12">
              <svg
                ...
                className="h-12 w-12 text-[#d2b48c]"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
            ...
            <span className="text-sm text-[#a98467]">
      ...
      {files && files.length > 0 && (
        <div className="mb-6">
          ...
          >
            <span>{uploading ? "Uploading..." : "Share Your Memories"}</span>
          </button>
      ...
      {uploading && (
        <div className="mb-6">
          ...
          <p className="text-center text-sm text-[#5e503f] mt-2">
